     ...
     <el-main>
       <div class="maindiv">
         <el-row :gutter="20" class="pXiah" style="margin-bottom:28px;">
          <el-col :span="20"><div class="grid-content bg-purple"><input placeholder="输入验证码" class="inpIphone" v-model.number="sendMterrcode" maxlength="4"></div></el-col>
          <timer-btn style="top: 0px;height: 25px;line-height: 40px;"  @run="uptet" ref="timer"></timer-btn>
        </el-row>
         <el-row :gutter="20" class="pXiah">
          <el-col :span="24"><div class="grid-content bg-purple"><input @keyup="checkPasswd" style="width:70%;" placeholder="请输入新密码(6-20个字母或数字)" ref=""  class="inpIphone" v-model="passwd" :type="isPassword" maxlength="20"><i @click="changePwd" class="seepwd icon iconfont">&#xe62e;</i></div></el-col>
        </el-row>
        <el-row :gutter="20" class="pXiah">
          <el-col :span="20"><div class="grid-content bg-purple"><input @keyup="checkPasswd" placeholder="确认密码"  class="inpIphone" v-model="repasswd" :type="isPassword" maxlength="20" @blur="checkPasswd"></div></el-col>
        </el-row>
        <div class="tipmsg" style="top: 14px;margin-left: -21px;position: relative;">{{tipmsg}}</div>
        <el-row :gutter="20" class="btnr" style="">
          <el-col :span="24"><div class="grid-content bg-purple"><el-button type="danger" :class="{isVoidActive:!isVoidActive}"  @click="spaStatus" style="margin-left: -14px;background:#10cb7e;border:0px;width:100%;text-align:center">确定</el-button></div></el-col>
        </el-row>
       </div>
     </el-main>
...
import {updatePwd} from "@/api/Registerlogo"
export default {
  name: 'changePwdPage',
  components:{
    publicJs,
  ...
  methods:{
    ...
    var url = this.$refs.publicjs.server_customer_url+"/customerSms/getVerificationCode";
      var params={
    ...
    spaStatus(){
      ...
      updatePwd(this.cellPhone,this.sendMterrcode,this.mtmsgid,this.passwd).then(res =>{
         if(res.data.returnCode === 1){
          this.tips().success('密码修改成功')
